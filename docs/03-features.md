...
`ambi`) are **global**: every route carries them. See `docs/04`.
...
Five cards — no rollout card, no fake sixth:
...
tooltip notes the BSD+2×width approximation.
...
- Extra columns `c2`, `c3`, `c4` are optional compact tuples
  `ring,cog,wheel,tire,crank,ambi`. Compact; no holes (`c3` without `c2`
  becomes `c2`).
...
- Add column: copy of column 1 into the next empty slot (max 4). Cannot
  remove column 1.
- Per-column **compact header**: number steppers for ring/cog/tire,
...
- Table of all derived metrics (the five cards + cadence-at-90). Delta
  ...
  better). No gear-inch target in v1.
...
- Load writes the global search and **navigates to `/`**.
...
- Export/import as JSON: `{ v: 1, setups: [...] }`. Unknown `v` refuses
  ...
  the rest **merge** (new ids append; same id overwrites).
